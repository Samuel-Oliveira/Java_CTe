...
import br.com.swconsultoria.cte.schema_300.cteModalRodoviarioOS.RodoOS;
...
import br.com.swconsultoria.cte.schema_300.cteOS.TUFSemEX;
import br.com.swconsultoria.cte.schema_300.cteOS.TUf;
...
public class Teste {
	...
	public static void main(String[] args) throws FileNotFoundException, CertificadoException {
		//Certificado certificado = CertificadoService.certificadoPfx("D:\\simetris\\emissao\\soteca.pfx", "");
		Certificado certificado = CertificadoService.certificadoPfx("D:\\simetris\\emissao\\lazarin.pfx", "");
		ConfiguracoesCte config = ConfiguracoesCte.criarConfiguracoes(EstadosEnum.SC, AmbienteEnum.HOMOLOGACAO,
				certificado, "C:\\Users\\progr\\Downloads\\PL_CTe_400");
	...
	public static void consultar(ConfiguracoesCte config) {
		try {
			// Veja
			// https://github.com/Samuel-Oliveira/Java_CTe/wiki/Configura%C3%A7%C3%B5es-CTe
	...
	public static void cancelar(ConfiguracoesCte config) {
		try {
	...
	public static void autorizar(ConfiguracoesCte config) {
		try {
			...
			String cnpj = "28116258000177"; // TODO Preencha o Cnpj
			String modelo = "57";
			int serie = 1;
			int numero = 1;
			...
			ChaveUtil chaveUtil = new ChaveUtil(config.getEstado(), cnpj, modelo, serie, numero, tipoEmissao, cct,
					LocalDateTime.now());
			...
			ide.setCCT(cct);
			ide.setCFOP("5932");
			ide.setNatOp("PRESTAÇÃO DE SERVIÇO DE TRANSPORTE INICIADA EM UNIDADE DA FE");
			ide.setMod(modelo);
			...
			ide.setProcEmi("0");
			ide.setVerProc("1.0");
			ide.setCMunEnv("5102504");
			ide.setXMunEnv("CÁCERES");
			ide.setUFEnv(br.com.swconsultoria.cte.schema_300.enviCTe.TUf.valueOf("MT"));
			ide.setModal("01");
			ide.setTpServ("0");
			ide.setCMunIni("5102504");
			ide.setXMunIni("CÁCERES");
			ide.setUFIni(br.com.swconsultoria.cte.schema_300.enviCTe.TUf.valueOf("MT"));
			ide.setCMunFim("5107602");
			ide.setXMunFim("RONDONÓPOLIS");
			ide.setUFFim(br.com.swconsultoria.cte.schema_300.enviCTe.TUf.valueOf("MT"));
			ide.setRetira("1");
			ide.setIndIEToma("1");
			...
			compl.setXObs(
					"FORMA DE PAGAMENTO DEPOSITO BANCARIO,FAVOR ENVIAR COPIA DO COMPROVANTE PARA O TRANSPORTADOR E ANEXAR VIA A DA CONTABILIDADE");
			infCTe.setCompl(compl);

			TCTe.InfCte.Emit emit = new TCTe.InfCte.Emit();
			emit.setCNPJ(cnpj);
			emit.setIE("137451750");
			emit.setXNome("SOTECA SERVICOS FLORESTAIS LTDA");
			emit.setXFant("SOTECA SERVICOS FLORESTAIS LTDA");
			emit.setCRT("1");
			...
			enderEmit.setCMun("5102504");
			enderEmit.setXMun("CÁCERES");
			enderEmit.setUF(br.com.swconsultoria.cte.schema_300.enviCTe.TUFSemEX.valueOf("MT"));
			...
			enderRem.setCMun("5102504");
			enderRem.setXMun("CÁCERES");
			enderRem.setUF(br.com.swconsultoria.cte.schema_300.enviCTe.TUf.MT);
			...
			dest.setCNPJ("05922863000130");
			dest.setIE("152846492");
			...
			enderDest.setCMun("1501402");
			enderDest.setXMun("BELÉM");
			enderDest.setUF(br.com.swconsultoria.cte.schema_300.enviCTe.TUf.PA);
			...
			infCTeSupl.setQrCodCTe(ObjetoCTeUtil.criaQRCode(cte.getInfCte().getId().substring(3), config));
			 ...
			 * throw new CteException("Status:" + retorno.getCStat() + " - Motivo:" +
			 * retorno.getXMotivo()); }
			 * 
			 * 
			 * 
			 * br.com.swconsultoria.cte.schema_300.retConsReciCTe.TRetConsReciCTe
			 * retornoCte; while (true) { retornoCte = Cte.consultaRecibo(config, recibo);
			 * if (retornoCte.getCStat().equals("105")) { System.out.
			 * println("Lote Em Processamento, vai tentar novamente apos 2 Segundo.");
	...
	public static void enviarCteOs(ConfiguracoesCte config) {
		try {
			...
			TCTeOS cte = new TCTeOS();
			cte.setVersao(ConstantesCte.VERSAO.CTE);
			...
			TCTeOS.InfCte.Compl compl = new TCTeOS.InfCte.Compl();
			compl.setXCaracSer("TRANSPORTE DE PESSOAS");
			compl.setXObs(
					"FORMA DE PAGAMENTO DEPOSITO BANCARIO,FAVOR ENVIAR COPIA DO COMPROVANTE PARA O TRANSPORTADOR E ANEXAR VIA A DA CONTABILIDADE");
			infCTe.setCompl(compl);
			...
			br.com.swconsultoria.cte.schema_300.cteOS.TEndeEmi enderEmit = new br.com.swconsultoria.cte.schema_300.cteOS.TEndeEmi();
			enderEmit.setXLgr("AV SANTO ANTONIO & CIA");
			enderEmit.setNro("0");
			enderEmit.setXCpl("QD 17 LT 01-02-03");
			enderEmit.setXBairro("PQ STO ANTONIO");
			...
			enderEmit.setXMun("CÁCERES");			
			enderEmit.setCEP("72900000");
			enderEmit.setUF(TUFSemEX.SC);

			emit.setEnderEmit(enderEmit);
			infCTe.setEmit(emit);
			

			TCTeOS.InfCte.VPrest prest = new TCTeOS.InfCte.VPrest();
			prest.setVTPrest("410.00");
			prest.setVRec("410.00");

			TCTeOS.InfCte.VPrest.Comp comp = new TCTeOS.InfCte.VPrest.Comp();
			comp.setVComp("500.00");
			comp.setXNome("TESTE");
			prest.getComp().add(comp);
			infCTe.setVPrest(prest);
			...
			br.com.swconsultoria.cte.schema_300.cteOS.TImpOS icms = new br.com.swconsultoria.cte.schema_300.cteOS.TImpOS();
			br.com.swconsultoria.cte.schema_300.cteOS.TImpOS.ICMSSN icmssn = new br.com.swconsultoria.cte.schema_300.cteOS.TImpOS.ICMSSN(); 
		icmssn.setCST("90");
		icmssn.setIndSN("1");
		...
		imp.setICMS(icms);
			...
			TCTeOS.InfCte.InfCTeNorm.InfServico ser = new TCTeOS.InfCte.InfCTeNorm.InfServico();
			ser.setXDescServ("TRANSPORTE DFE PESSOAS");
			infCTeNorm.setInfServico(ser);
			
			TCTeOS.InfCte.InfCTeNorm.Seg seg = new TCTeOS.InfCte.InfCTeNorm.Seg();
			seg.setRespSeg("4");
			seg.setXSeg("XXXXX");
			seg.setNApol("999");
			...
			RodoOS rodo = new RodoOS();
			rodo.setTAF("00418422");

			infModal.setAny(ObjetoCTeUtil.objectToElement(rodo, RodoOS.class, "rodoOS"));
			...
			cte.setInfCte(infCTe);
			// MOnta e Assina o XML
			cte = Cte.montaCteOS(config, cte, false);
			...
			// Envia a Cte para a Sefaz
			br.com.swconsultoria.cte.schema_300.retCTeOS.TRetCTeOS retorno = Cte.enviarCteOS(config, cte);
