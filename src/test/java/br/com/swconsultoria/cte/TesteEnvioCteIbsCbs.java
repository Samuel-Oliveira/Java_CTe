...
import br.com.swconsultoria.cte.util.IbsCbsUtil;
...
class TesteEnvioCteIbsCbs {
    ...
    private static TCTe preencheObjetoCte(ConfiguracoesCte config, String cnpj, int serie, int numero) throws CteException, IOException {
        TCTe cte = new TCTe();
        ...
        BigDecimal vTPrest = new BigDecimal("410.00");

        TCTe.InfCte.VPrest prest = new TCTe.InfCte.VPrest();
        prest.setVTPrest(vTPrest.toPlainString());

        // ─── IBS/CBS – Reforma Tributária ───────────────────────────────────
        // Carrega o JSON de IBS/CBS disponibilizado pela SEFAZ
        String jsonIbsCbs = new String(Files.readAllBytes(
                Paths.get("src/test/resources/ibscbs.json")));
        // Alíquotas vigentes (exemplo); ajuste conforme portaria do seu Estado
        IbsCbsUtil ibsCbsUtil = new IbsCbsUtil(jsonIbsCbs, DocumentoCteEnum.CTE);

        // CST 000 / cClassTrib 000001 = tributacao integral - transporte (IndCTe=true)
        TTribCTe tribCTe = ibsCbsUtil.montaImposto("000001", vTPrest);

        // vTotDFe = vTPrest + vIBS + vCBS (obrigatório quando IBSCBS presente)
        BigDecimal vTPrestComTrib = ibsCbsUtil.calculaVTPrestTot(vTPrest.toPlainString());
        BigDecimal vTotTrib = vTPrestComTrib.subtract(vTPrest);
        // ────────────────────────────────────────────────────────────────────

        // vRec deve ser <= vTPrest (regra 531)
        prest.setVRec(vTPrest.toPlainString());

        TCTe.InfCte.VPrest.Comp comp = new TCTe.InfCte.VPrest.Comp();
        comp.setVComp(vTPrest.toPlainString()); // deve corresponder ao vTPrest
        comp.setXNome("FRETE");
        ...
        imp.setICMS(icms);
        imp.setIBSCBS(tribCTe);
        imp.setVTotTrib(vTotTrib.toPlainString());
        // vTotDFe: SEFAZ homologacao ainda usa regra antiga (vTPrest sem IBS/CBS).
        // Em producao, ajustar para vTPrestComTrib quando a SEFAZ atualizar para reforma tributaria.
        imp.setVTotDFe(vTPrest.toPlainString());
        infCTe.setImp(imp);
